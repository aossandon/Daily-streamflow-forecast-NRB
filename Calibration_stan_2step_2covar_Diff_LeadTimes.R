...
LeadT=1#enter the lead time 1-10 days
...
dir_create(paste("Results/",LeadT,"LT",sep = ""))
dir_JPG=paste(dir_r,"/",LeadT,"LT",sep = "")
...
datath=data[[paste("VIC_",LeadT,"d",sep="")]][,4:8]
...
#get index with missing values
No=length(ind_o)
stan_data = list(N=nrow(data$Q),No=No,S=5,ind_o=ind_o,y=datQ[,4:8],covQ=datath1,covP=covP,covQs=as.matrix(datath),covPs=covPs,thrQ=thrQ,indc=1:5)
#get the element for each step
nam=c("g","m","h","ho","s")
for (i in 1:5) {
  if (LeadT==0 & i==3 & length(which(stan_data$covP[,i]==0))>0){
...
val=list(des="")
N=dim(Simul$mu_han)[2]
nsim=dim(Simul$mu_han)[1]
nam1=c()
sig=c()
...
traceplot(stanfit, pars = c(paste("mu_mand","[",i,"]",sep=""),"lp__"), inc_warmup = T, nrow = 3)
p1.base <- recordPlot()
invisible(dev.off())
#save the figure
setwd(dir_JPG)
jpeg("traceplot_mu_mand.jpg", width =7, height = 6, units = 'in', res = 300)
p1.base
dev.off()
...
print(Val)
setwd(dir_JPG)
write.table(Val, file = "Metrics_summary.txt",sep="\t")
